...
export function DriverSignupForm() {
  ...
  async function handleFormSubmit(values: SignupValues) {
    ...
    const res = await sendRegistrationOtpApi(values.email, values.name, "DRIVER");
  ...
  async function handleResendCode() {
    ...
    const res = await sendRegistrationOtpApi(savedValues.email, savedValues.name, "DRIVER");
    setResending(false);
    ...
    return (
      <form onSubmit={handleVerifyAndRegister} className="mt-2 space-y-4">
        ...
        >
          ...
          {verifying ? "Verifying & Creating Account…" : "Verify & Create Driver Account"}
        </Button>
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} noValidate className="space-y-1">
      ...
      <p className="mt-4 text-center text-sm text-muted-foreground">
        ...
        <Link href="/driver-login" className="font-semibold text-primary hover:text-primary/80 transition-colors">
          Sign in
        </Link>
      </p>
    </form>
